...
def build_features(
    ...
    user_avg = float(profile.get("avg_amount") or 0) or POPULATION_MEDIAN_AMOUNT
    ...
    return {
        ...
        # An unknown payee is new to us, but we must not claim it is a
        # zero-day-old account with no payers - that is a fabricated signal.
        "payee_is_new": 0.0 if (reputation.get("repeat_payers") or 0) > 0 else 1.0,
        "payee_age_days": float(reputation.get("age_days") or 0) if known else 180.0,
        "payee_distinct_payers": float(reputation.get("distinct_payers") or 0) if known else 8.0,
        "payee_repeat_ratio": (
