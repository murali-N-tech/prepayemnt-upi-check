...
const DECISION_STYLE: Record<string, { ring: string; text: string; Icon: typeof ShieldCheck }> = {
  APPROVE: { ring: "bg-emerald-500/10 border-emerald-500/20", text: "text-emerald-400", Icon: ShieldCheck },
  WARN:    { ring: "bg-amber-500/10 border-amber-500/20",     text: "text-amber-400",   Icon: AlertTriangle },
  STEP_UP: { ring: "bg-orange-500/10 border-orange-500/20",   text: "text-orange-400",  Icon: ShieldAlert },
  BLOCK:   { ring: "bg-rose-500/10 border-rose-500/20",       text: "text-rose-400",    Icon: ShieldX },
};
...
const SEVERITY_STYLE: Record<string, string> = {
  ...
  info: "border-slate-700 bg-slate-950/60 text-slate-400",
};
...
export default function PayeeCheck() {
  ...
  return (
    <div className="space-y-8 animate-fade-in" id="payee-check-container">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Check a payee</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-fit">
          <h2 className="text-xl font-semibold text-white mb-4">Payee details</h2>
          ...
          >
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white placeholder-slate-600 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              {EXAMPLES.map((ex) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-sm text-slate-300 bg-slate-950 hover:bg-slate-800/60 border border-slate-800 rounded-lg transition"
                >
                  {ex.label}
                </button>
              ))}
          ...
          {result && style ? (
            <>
              <div className={`border rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 ${style.ring}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-300 text-sm mt-1">{result.headline}</p>
                    {result.payee.vpa && (
                ...
                <div className="text-center shrink-0">
                  <div className="text-xs text-slate-400 uppercase font-semibold">Payee risk</div>
                  <div className="text-5xl font-black text-white mt-1">{result.risk_score}</div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-white mb-4">What the check found</h3>
                <ul className="space-y-2.5">
                  ...
                  {result.findings.length === 0 && (
                    <li className="text-sm text-slate-400">Nothing stood out about this payee.</li>
                  )}
                </ul>
              </div>
              ...
              {/* Reputation */}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-white mb-1">Payee history</h3>
                ...
                {rep?.known ? (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      <div key={label} className="bg-slate-950 border border-slate-800/60 rounded-lg p-3">
                        <Icon className="h-4 w-4 text-indigo-400 mb-2" />
                  ...
                  <div className="flex items-start gap-3 text-sm text-slate-400 bg-slate-950 border border-slate-800 rounded-lg p-4">
                ...
                {result.payee.vpa && (
                  <button
                    onClick={reportPayee}
                    disabled={reported}
                    className="mt-5 flex items-center gap-2 text-sm px-4 py-2 rounded-lg border border-slate-700 text-slate-300 hover:bg-slate-800/60 disabled:opacity-50 disabled:cursor-default transition"
                  >
              ...
              {/* Score breakdown */}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="text-sm font-semibold text-white mb-4">Where the score came from</h3>
                  ...
                  {[
                    ...
                    <div key={label as string}>
                      <div className="flex justify-between text-xs text-slate-400 mb-1">
                        <span>{label}</span>
                        <span className="font-mono text-slate-300">{value as number}</span>
                      </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-12 text-center h-full flex flex-col justify-center items-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-1">Check before you pay</h3>
