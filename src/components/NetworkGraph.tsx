  ...
  return (
    <div className="space-y-8 animate-fade-in" id="network-graph-container">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-ink mb-2">Fraud Network Graph</h1>
        ...
        <button
          onClick={fetchData}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-surface hover:bg-raised text-ink-muted font-medium rounded-lg border border-line transition shadow-sm h-fit self-start md:self-auto"
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        {/* Left Stats Column */}
        <div className="bg-surface border border-line rounded-xl p-5 space-y-6 xl:col-span-1">
          <h3 className="font-semibold text-ink">Network Telemetry</h3>

          <div className="space-y-4">
            <div className="p-3 bg-inset rounded-lg">
              <span className="text-xs text-ink-subtle block uppercase font-semibold">Total Nodes</span>
              <span className="text-3xl font-extrabold text-ink mt-1 block">{nodes.length}</span>
            </div>

            <div className="p-3 bg-inset rounded-lg">
              <span className="text-xs text-ink-subtle block uppercase font-semibold">Connection Links</span>
              <span className="text-3xl font-extrabold text-ink mt-1 block">{edges.length}</span>
            </div>

            <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-danger rounded-lg">
              <span className="text-xs text-danger block uppercase font-semibold">Graph anomalies</span>
              <span className="text-3xl font-extrabold text-danger mt-1 block">{suspiciousNodes.length}</span>
            </div>
          </div>

          <div className="pt-4 border-t border-line text-xs space-y-2 text-ink-muted">
        ...
        <div className="xl:col-span-3 bg-inset border border-line rounded-xl overflow-hidden relative min-h-[500px]">
          {loading ? (
            <div className="absolute inset-0 flex items-center justify-center bg-inset/80 z-10">
              <div className="h-10 w-10 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
            </div>
          ) : error ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
              <AlertCircle className="h-12 w-12 text-danger mb-2" />
              <p className="text-ink-muted">{error}</p>
            </div>
          ) : nodes.length === 0 ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
              <Share2 className="h-12 w-12 text-ink-faint mb-2" />
              <p className="text-ink-muted">No network data active. Run some transactions in the risk prediction panel first!</p>
            </div>
          ) : (
            <svg
              viewBox="0 0 800 500"
                ...
                {edges.map((e, idx) => {
                  ...
                  return (
                    <line
                      ...
                      x2={p2.x}
                      y2={p2.y}
                      stroke={isSuspicious ? chart.danger : chart.edge}
                    ...
                    <g key={node.id} transform={`translate(${pos.x}, ${pos.y})`}>
                      {node.type === "user" ? (
                        <circle
                          ...
                          fill={isSuspicious ? chart.danger : chart.brand}
                          stroke={isSuspicious ? chart.nodeStroke : chart.brand}
                          strokeWidth={2}
                        ...
                        <rect
                          ...
                          rx={3}
                          fill={isSuspicious ? chart.danger : chart.ok}
                          stroke={isSuspicious ? chart.nodeStroke : chart.ok}
                          strokeWidth={2}
                          className="transition-all hover:scale-125 duration-150"
                        />
                      )}
      ...
      {suspiciousDetails.length > 0 && (
        <div className="bg-surface border border-line rounded-xl p-6">
          <h3 className="text-lg font-semibold text-ink mb-1">Why these were flagged</h3>
          ...
          <ul className="space-y-3">
            {suspiciousDetails.map((d) => (
              <li key={d.node} className="bg-inset border border-line rounded-lg p-3">
                <div className="font-mono text-sm text-danger mb-1.5">{d.node}</div>
